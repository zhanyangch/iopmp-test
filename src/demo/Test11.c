...
int main(void)
{
	...
	printf("\nQEMU IOPMP testcase11\n");

	srcmd_fmt = iopmp_ops->get_srcmd_fmt(DEV_IOPMP0);
	mdcfg_fmt = iopmp_ops->get_mdcfg_fmt(DEV_IOPMP0);

	if (!iopmp_ops->get_sps_en(DEV_IOPMP0)) {
		printf("Failure: This case only support sps_en\n");
		while (1);
	}
	...
	*(volatile unsigned int *)&CPU_WO_Array[0] = 0x7777CCCC;
	...
	/* Setup IOPMP */
        ...
        /* CPU_MD[0] region 1: CPU_RO_Array ~ (CPU_RO_Array + ARRAY_SIZE), RWX */
        iopmp_ops->napot_config(DEV_IOPMP0, CPU_MD[0] * iopmp_k, (void *)CPU_RO_Array, ARRAY_SIZE, ENTRY_CFG_XWR(ENTRY_X_ON, ENTRY_W_ON, ENTRY_R_ON));
        /* CPU_MD[0] sps RO */
        iopmp_ops->srcmd_r_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[0]);
        ...
        iopmp_ops->napot_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k, (void *)CPU_WO_Array, ARRAY_SIZE, ENTRY_CFG_XWR(ENTRY_X_ON, ENTRY_W_ON, ENTRY_R_ON));
        /* CPU_MD[1] sps WO */
        iopmp_ops->srcmd_w_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[1]);
        ...
        /* CPU_MD[2] sps RW */
        iopmp_ops->srcmd_r_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[2]);
        iopmp_ops->srcmd_w_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[2]);

	int error_reaction_ctrl = (ERR_CFG_CTRL_IE_ENABLE | ERR_CFG_CTRL_RS_DISABLE); //ERR_CFG_CTRL_IE_ENABLE
	...
	*(volatile unsigned int *)&CPU_RO_Array[0] = 0x7777CCCC;
	...
	printf("IOPMP testcase11 success.\n");
	while(1);
}
