...
/*
 * Testcase 17:
 * Test IOPMP MSI failure
 * IOPMP_MSI_RRID(1) send msi without enough permission
 */
...
#define SELECT_EIDELIVERY     0x70
#define SELECT_EITHRESHOLD    0x72
#define SELECT_EIP0           0x80
#define SELECT_EIE0           0xC0

/* M-mode imsic indirect R/W */
void imsic_indirect_write(int csr, long value) {
	write_csr(CSR_MISELECT, csr);
	write_csr(CSR_MIREG, value);
}

long imsic_indirect_read(int csr) {
	write_csr(CSR_MISELECT, csr);
	return read_csr(CSR_MIREG);
}
...
void iopmp_irq_init() {
	imsic_indirect_write(SELECT_EIDELIVERY, 1);
	imsic_indirect_write(SELECT_EITHRESHOLD, 0xFF);
	/* IOPMP */
	imsic_indirect_write(SELECT_EIE0, (1 << IRQ_IOPMP_SOURCE));
}
...
{
	uart_init();
	printf("\nQEMU IOPMP testcase17\n");
	printf("msi_werr test\n");
	...
	if (srcmd_fmt == 2) {
		printf("This case dose not support srcmd_fmt2\n");
		while (1);
	}
	...
	*(volatile unsigned int *)&CPU_WO_Array[0] = 0x7777CCCC;
	...
	if(srcmd_fmt == 0) {
		...
		iopmp_ops->srcmd_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[0]);
		/* CPU_MD[0] region 1: CPU_RO_Array ~ (CPU_RO_Array + ARRAY_SIZE), no executable, no writable, readable */
		iopmp_ops->off_config(DEV_IOPMP0, CPU_MD[0] * iopmp_k, (void *)CPU_RO_Array);
		iopmp_ops->tor_config(DEV_IOPMP0, CPU_MD[0] * iopmp_k + 1, (void *)&CPU_RO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_OFF, ENTRY_R_ON));

		iopmp_ops->srcmd_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[1]);
		/* CPU_MD[1] region 1: CPU_WO_Array ~ (CPU_WO_Array + ARRAY_SIZE), no executable, writable, no readable */
		iopmp_ops->off_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k, (void *)CPU_WO_Array);
		iopmp_ops->tor_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k + 1, (void *)&CPU_WO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_ON, ENTRY_R_OFF));
	...
	int error_reaction_ctrl = (ERR_CFG_CTRL_IE_ENABLE | ERR_CFG_CTRL_RS_DISABLE | ERR_CFG_CTRL_MSI_EN_ENABLE);
	...
	(void)data;
	/* Wait msi write error */
	while (!iopmp_ops->get_msi_werr(DEV_IOPMP0));
	printf("Get a msi write error\n");
	iopmp_ops->clear_msi_werr(DEV_IOPMP0);
	if(iopmp_ops->get_msi_werr(DEV_IOPMP0)) {
		printf("Failure: msi write error bit is not cleared\n");
	}

	/* MSI write blocked by IOPMP generate the subsequent violation */
	int err_mfr = iopmp_ops->get_err_mfr(DEV_IOPMP0);
	if(!(err_mfr & 0x1 << IOPMP_MSI_RRID)) {
		printf("err_mfr: 0x%x\n", err_mfr);
		printf("Failure: MFR record error\n");
	}

	printf("IOPMP testcase17 success.\n");
	while(1);
}
