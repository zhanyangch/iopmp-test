...
/*
 * Testcase 6:
 * Suppress bus error and irq interrupt by entry config
 *
 */
...
int main(void)
{
	...
	*(volatile unsigned int *)&CPU_WO_Array[0] = 0x7777CCCC;
	...
	/* CPU_MD[0] region 1: CPU_RO_Array ~ (CPU_RO_Array + ARRAY_SIZE), no executable, no writable, readable, Suppress write error */
	iopmp_ops->napot_config(DEV_IOPMP0, CPU_MD[0] * iopmp_k + 1, (void *)CPU_RO_Array, ARRAY_SIZE, ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_OFF, ENTRY_R_ON) | ENTRY_CFG_SIE_XWR(0, 1, 0) | ENTRY_CFG_SEE_XWR(0, 1, 0));

	iopmp_ops->srcmd_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[1]);
	/* CPU_MD[1] region 1: CPU_WO_Array ~ (CPU_WO_Array + ARRAY_SIZE), no executable, writable, no readable, Suppress read error */
	iopmp_ops->napot_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k, (void *)CPU_WO_Array, ARRAY_SIZE, ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_ON, ENTRY_R_OFF)| ENTRY_CFG_SIE_XWR(0, 0, 1) | ENTRY_CFG_SEE_XWR(0, 0, 1));
	...
	iopmp_ops->tor_config(DEV_IOPMP0, CPU_MD[0] * iopmp_k + 3, (void *)&CPU_RO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_OFF, ENTRY_R_ON) | ENTRY_CFG_SIE_XWR(0, 1, 0) | ENTRY_CFG_SEE_XWR(0, 1, 0));

	iopmp_ops->srcmd_add_md(DEV_IOPMP0, CPU_RRID, CPU_MD[1]);
	/* CPU_MD[1] region 1: CPU_WO_Array ~ (CPU_WO_Array + ARRAY_SIZE), no executable, writable, no readable, Suppress read error */
	iopmp_ops->off_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k + 4, (void *)CPU_WO_Array);
	iopmp_ops->tor_config(DEV_IOPMP0, CPU_MD[1] * iopmp_k + 5, (void *)&CPU_WO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_ON, ENTRY_R_OFF) | ENTRY_CFG_SIE_XWR(0, 0, 1) | ENTRY_CFG_SEE_XWR(0, 0, 1));
	...
	iopmp_ops->tor_config(DEV_IOPMP0, CPU_MD[2] * iopmp_k + 1, (void *)0xFFFFFFFF, ENTRY_CFG_XWR(ENTRY_X_ON, ENTRY_W_ON, ENTRY_R_ON));
		...
		printf("Configure IOPMP entries with TOR scheme.\n");
                /* CPU_MD[1] region 1: CPU_WO_Array ~ (CPU_WO_Array + ARRAY_SIZE), no executable, no writable, readable , Suppress write error */
		iopmp_ops->off_config(DEV_IOPMP0, CPU_RRID * iopmp_k + 0, (void *)CPU_RO_Array);
		iopmp_ops->tor_config(DEV_IOPMP0, CPU_RRID * iopmp_k + 1, (void *)&CPU_RO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_OFF, ENTRY_R_ON) | ENTRY_CFG_SIE_XWR(0, 1, 0) | ENTRY_CFG_SEE_XWR(0, 1, 0));

		/* CPU_MD[1] region 1: CPU_WO_Array ~ (CPU_WO_Array + ARRAY_SIZE), no executable, writable, no readable , Suppress read error */
		iopmp_ops->off_config(DEV_IOPMP0, CPU_RRID * iopmp_k + 2, (void *)CPU_WO_Array);
		iopmp_ops->tor_config(DEV_IOPMP0, CPU_RRID * iopmp_k + 3, (void *)&CPU_WO_Array[ARRAY_SIZE], ENTRY_CFG_XWR(ENTRY_X_OFF, ENTRY_W_ON, ENTRY_R_OFF) | ENTRY_CFG_SIE_XWR(0, 0, 1) | ENTRY_CFG_SEE_XWR(0, 0, 1));
        ...
        /* Global: enable bus error and interrupt */
	...
	*(volatile unsigned int *)&CPU_RO_Array[0] = 0x7777CCCC;
	...
	printf("IOPMP testcase6 success.\n");
	while(1);
}
