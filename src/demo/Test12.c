...
/*
 * Testcase 12:
 * Error Reaction: Bus error, No IOPMP IRQ
 * Read error x 2
 * Check MFR bits
 * Handle IRQ manually
 */
...
int main(void)
{
	...
	printf("\nQEMU IOPMP testcase12\n");

	srcmd_fmt = iopmp_ops->get_srcmd_fmt(DEV_IOPMP0);
	mdcfg_fmt = iopmp_ops->get_mdcfg_fmt(DEV_IOPMP0);

	if (!iopmp_ops->get_mfr_en(DEV_IOPMP0)) {
		printf("This case only support MFR\n");
		while (1);
	}
	...
	*(volatile unsigned int *)&CPU_WO_Array[0] = 0x7777CCCC;
	...
	int error_reaction_ctrl = (ERR_CFG_CTRL_RS_DISABLE | ERR_CFG_CTRL_IE_DISABLE);
	...
	while (!bus_error_done);

	printf("\nTry to 2nd read data from WO region.\n");
	bus_error_done = 0;
	expect_exception_cause = TRAP_M_L_ACC_FAULT;
	asm volatile("" ::: "memory");
	data = *(volatile unsigned int *)CPU_WO_Array;
	while (!bus_error_done);

	int err_mfr = iopmp_ops->get_err_mfr(DEV_IOPMP0);
	if(!(err_mfr & 0x1 << CPU_RRID)) {
		printf("err_mfr: 0x%x\n", err_mfr);
		printf("Failure: MFR record error\n");
	}
	printf("Clear Error pending bit manually!\n");
	iopmp_irq_handler();
	while (!iopmp_isr_done);
	if (data == 0x7777CCCC) {
		printf("Failure: WO Data %x\n", data);
		while (1);
	}

	printf("IOPMP testcase12 success.\n");
	while(1);
}
